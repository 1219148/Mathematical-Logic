...
在后续同步 GitHub 更新后，感知模块对玩家所属 tile 的判定方式发生了变化：新版 `PerceptionEngine` 更倾向于使用 CNN 输出的语义 grid 中的 player tile，而旧版更依赖 player heatmap center 推出的 tile。这个变化提高了部分静态识别稳定性，但也暴露出策略层原本对感知输出的隐含假设：低层移动控制假设玩家 tile 会随移动连续变化，房间记忆假设开箱、桥切换等动态对象不会改变房间身份。实际调试中，t4 会在取剑后因为已开宝箱短暂被重新识别而反复处理同一位置；t5 会在按钮激活后把中心房中的单格 bridge 当成桥房间，从而错误地把普通地面都视为 blocked，导致无法规划到西侧出口。

因此我做了三类兼容修复，目标是提高符号抽象的一致性，而不是写死某个关卡方向：

1. 玩家位置稳定化：策略层仍使用感知模块输出，但移动控制优先使用 `player_entity.center_px` 推出的 tile，并在低置信度或非法 tile 时回退。这避免了 grid tile 在交互动画、房间边界和开箱后的短暂抖动影响 A*/BFS。
2. 房间签名去动态化：`room_signature` 不再把 transient chest、bridge、gap 作为房间身份的一部分。宝箱是否开过、桥是否存在仍会作为 room feature 或 memory 内容参与决策，但不再导致同一物理房间被记成多个房间。
3. 桥约束语义化：不再使用“只要看见 bridge 就封锁非 bridge tile”的规则，也不使用 `len(bridge_tiles) > 1` 这类数量 heuristic。当前规则是：只有 bridge 邻接 gap，或 bridge 连接两个及以上出口方向时，才把它视为必须受限通行的桥结构。这样 t4 的中央桥会触发桥约束，t5 中心房的单格通路不会误封锁普通地面。
...
- 对感知抖动的鲁棒性：房间身份由相对稳定的出口、墙体和 NPC 结构决定，动态对象只作为当前特征或记忆内容；玩家 tile 由 grid 与 heatmap center 组合得到，避免单一感知字段错误放大到全局规划。
...
这个修复后，当前公开环境下 task 1 到 task 5 在 seed 0 的统一测评中全部通关；task 5 能打开 4 个宝箱并触发 `world_completed=True`，且没有破坏 t1-t4。
...
- 感知接口更新后的兼容调试：同步新版 perception 后，t4/t5 一度不能通过。诊断发现，失败不是高层目标完全错误，而是玩家 tile 来源、动态 chest/bridge/gap 对房间签名的影响，以及 bridge blocker 规则过强共同导致的。最终保留的修复是语义化桥约束、稳定玩家 tile、去除动态对象对房间身份的影响，以及在出口选择中优先未完成或仍有未知 frontier 的房间。
